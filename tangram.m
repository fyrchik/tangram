...
% element is discriminated union of 2 operations:
...
:- type element ---> step(int,int); turn(int).
...
:- func left(int) = element.
left(A) = turn(A rem 360).

:- func right(int) = element.
right(A) = turn((-A) rem 360).

:- pred add_turns(element::in, element::in, element::out) is semidet.
add_turns(turn(Deg1),turn(Deg2), turn((Deg1 + Deg2) rem 360)).

:- pred append_turns(element::in, element::in, element::out) is semidet.
append_turns(turn(Deg1), turn(Deg2), turn((Deg1+Deg2+180) rem 360)).

:- pred add_steps(element::in, element::in, element::out) is semidet.
add_steps(step(A1,B1),step(A2,B2), step(A1+A2,B1+B2)).

:- func normalize(list(element)) = list(element).
...
% Elements are combined when we put them next to each other.
% This means:
...
% :- pred combine_at(list(element)::in, int::in, list(element)::in, int::in, list(element)::out) is nondet.
% combine_at([E|Es],I1,F2,I2,F) = X :-
%   if combine_at(Es,)

:- pred sub_turns(element::in, element::in, element::out) is semidet.
sub_turns(turn(Deg1), turn(Deg2), turn((Deg1-Deg2) rem 360)).

:- pred sub_steps(element::in, element::in, element::out) is semidet.
sub_steps(step(A1,B1), step(A2,B2), step(A1-A2,B1-B2)).

:- pred invert_turn(element::in, element::out) is semidet.
...
:- pred insert_after(list(element)::in, list(element)::in, list(element)::in, list(element)::out) is semidet.
